...
export default function SwipeDeck({ jobs, onApply, onSkip, onTapDetails, onResetDeck }: SwipeDeckProps) {
  return (
    <div className="relative w-full h-[540px] max-w-md mx-auto">
      {/* Ambient glow ring behind deck */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] h-[340px] rounded-full bg-indigo-500/6 blur-[80px]" />
      </div>
        ...
        {jobs.length > 0 ? (
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={onResetDeck}
              className="px-7 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-500 to-purple-600 text-[12.5px] font-bold text-white shadow-[0_4px_20px_rgba(99,102,241,0.25)] flex items-center gap-2"
            >
              <RefreshCcw className="w-3.5 h-3.5" /> Reset Deck
            </motion.button>
          </motion.div>
        )}
      </AnimatePresence>
